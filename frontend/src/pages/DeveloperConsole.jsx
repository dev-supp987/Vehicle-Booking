...
import { Shield, Lock, Edit, Check, X, Search, Key, Sliders } from 'lucide-react';
    ...
    const savePermissions = async () => {
        try {
            await axios.put(`/api/auth/users/${selectedUser.id}/permissions`, {
                ...
                isSuperAdmin: permissionsForm.isSuperAdmin // Handling Super Admin toggle here too if needed, or separately
            });
        ...
        return (
            <Layout>
                <div style={{ padding: '2rem', textAlign: 'center', color: 'var(--error)' }}>
                    <Shield size={48} />
                    <h2>Access Denied</h2>
                    <p>This area is restricted to Super Administrators.</p>
                </div>
            </Layout>
        );
    }
            ...
            <div className="page-header">
                <div>
                    <h1 style={{ fontSize: '1.5rem', fontWeight: '700', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        <Shield style={{ color: 'var(--primary)' }} /> Developer Access Console
                    </h1>
                    <p style={{ color: 'var(--text-secondary)' }}>Manage system-wide permissions and super admin privileges.</p>
                </div>
            </div>

            <div className="card" style={{ padding: 0 }}>
                <div style={{ padding: '1.5rem', borderBottom: '1px solid var(--border)', display: 'flex', gap: '1rem', alignItems: 'center' }}>
                    <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: '0.8rem', background: 'var(--background)', padding: '0.6rem 1rem', borderRadius: '8px', border: '1px solid var(--border)' }}>
                                    ...
                                    <td style={{ padding: '1rem' }}>
                                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.3rem' }}>
                                            {u.permissions && Object.keys(u.permissions).map(p => (
                                                u.permissions[p] && (
                                                    <span key={p} style={{ fontSize: '0.7rem', padding: '0.1rem 0.4rem', borderRadius: '4px', background: 'rgba(59, 130, 246, 0.1)', color: 'var(--primary)' }}>
                                                        {p}
                                                    </span>
                                                )
                                            ))}
